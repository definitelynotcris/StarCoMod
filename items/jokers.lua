...
function Game:update(dt)
    ...
    while StarCoMod.dtcounter >= 0.010 do
        ...
        if G.STAGE == G.STAGES.RUN and G.GAME.scptimer and (G.GAME.scptimer > 0) then decrementingTickEvent("G.GAME.scptimer",0) end 
        ...
        if G.GAME.scptimer ~= nil then --add time on ante clear
            if G.lastante == nil then G.lastante = G.GAME.round_resets.ante end
            if G.lastante < G.GAME.round_resets.ante then
                G.GAME.scptimer = G.GAME.scptimer + 180
                G.lastante = G.GAME.round_resets.ante
            end
        end

        if G.GAME.scptimer == 0 then 
            G.STATE = G.STATES.GAME_OVER
            G.STATE_COMPLETE = false
            G.GAME.scptimer = nil
        end

        --if G.GAME.round == 0 then G.GAME.scptimer = nil end
...
function modulate_sound(dt)
    ...
    G.ARGS.ambient_sounds.star_096scream = {
        volfunc = function(_prevvolume)
            ...
            if G.GAME.scptimer ~= nil and G.GAME.scptimer ~= 0 then 
            return ( (1/G.GAME.scptimer))
            else return 0 end
        end,
    }
end
...
------------D20------------
SMODS.Sound({key = "boom", path = "boom.mp3",})

SMODS.Atlas{
    key = 'd20', --atlas key
    path = 'd20.png', --atlas' path in (yourMod)/assets/1x or (yourMod)/assets/2x
    px = 71, --width of one card
    py = 95 -- height of one card
}
...
SMODS.Joker{
    ...
    loc_txt = { -- local text
        name = 'D20',
        text = {
          "Rolls for one of 20 random effects",
        },
    },
        config = {
        extra = {
    ...
    loc_vars = function(self, info_queue, center)
		return {    key = ( "j_star_d20_" .. G.desctab % 3), 
                    vars = { center.ability.extra.luck, center.ability.extra.roll,
                            center.ability.extra.triggered, center.ability.extra.handsizemod,
                            center.ability.extra.moneymod, center.ability.extra.rolled16,
                            center.ability.extra.bonusluck, center.ability.debuffedjoker,
    ...
    add_to_deck = function(self, card, from_debuff)
        G.GAME.starcoluck = G.GAME.starcoluck + (1 + card.ability.extra.bonusluck)
        G.hand:change_size(card.ability.extra.handsizemod)
    end,

    remove_from_deck = function(self, card, from_debuff)
        G.GAME.starcoluck = G.GAME.starcoluck - (1 + card.ability.extra.bonusluck)
        G.hand:change_size(-1 * card.ability.extra.handsizemod)
    end,

    update = function(self, card, front) --in case of rolling 14
        local _myid = getJokerID(card)
        if G.jokers and G.jokers.cards[1] then card.ability.extra.bptarget = G.jokers.cards[_myid + 1] end
    end,

    calculate = function(self, card, context)
        local ret = SMODS.blueprint_effect(card, G.jokers.cards[1], context)
        if ret and card.ability.extra.roll == 14 then
            ret.colour = G.C.RED
        ...
        --pick rolls here
        if context.before and context.cardarea == G.jokers and card.ability.extra.triggered == false then
            
            card.ability.extra.triggered = true
            card.ability.extra.roll = (math.random(1, 20))
            
            if card.ability.extra.roll == 1 then  --explode DONE
                SMODS.calculate_effect({message = "1", sound = 'star_boom', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)
                if not card.ability.eternal then
                    ...
                    G.E_MANAGER:add_event(Event {
                        ...
                        delay = 0.2, 
                        func = function() 
                            card:juice_up(juicesize,juicespin)
                            juicespin = juicespin + 0.05
                            juicesize = juicesize + 0.1
                            play_sound('tarot2', juicespin)
                            return true
                        end,
                    })
                    end    
                    G.E_MANAGER:add_event(Event {
                        ...
                        delay = 0.2, 
                        func = function()  
                            play_sound("star_itsbroken", 1, 0.7)
                            card:shatter()
                            return true
                        end,
                    })
            ...
            elseif card.ability.extra.roll == 2 then --debuff a random joker this hand DONE
                SMODS.calculate_effect({message = "2", sound = 'star_coinheads', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)
                if #G.jokers.cards > 1 then
                    ...
                    local target = pseudorandom_element(targets, pseudoseed("star_d20roll2"))
                    if target then
                        card.ability.debuffedjoker = getJokerID(target)
                        G.jokers.cards[card.ability.debuffedjoker]:juice_up()
                        G.jokers.cards[card.ability.debuffedjoker]:set_debuff(true)
                    end
                end

            elseif card.ability.extra.roll == 3 then --  -1 hand or -1 discard till round end DONE
                SMODS.calculate_effect({message = "3", sound = 'star_coinheads', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)
                G.E_MANAGER:add_event(Event {
                  trigger = "after",
                  delay = 0, 
                  func = function() 
                        if (math.random(1, 2)) == 2 and G.GAME.current_round.discards_left > 0 then 
                            ease_discard(-1, true, true)
                        else 
                            ease_hands_played(-1, true, true)
                        end
                    return true
                  end,
                })
            ...
            elseif card.ability.extra.roll == 4 then -- destroy 3 random cards held in hand DONE
                SMODS.calculate_effect({message = "4", sound = 'star_coinheads', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)
                local todestroy = pickrandomfromhand(3)
            ...
            elseif card.ability.extra.roll == 5 then --   -1 hand size till round end DONE 
                SMODS.calculate_effect({message = "5", sound = 'star_coinheads', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)
                card.ability.extra.handsizemod = card.ability.extra.handsizemod - 1
                G.hand:change_size(-1)

            elseif card.ability.extra.roll == 6 then -- Increase or decrease a random hand's level by 2 DONE
                SMODS.calculate_effect({message = "6", sound = 'star_coinheads', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)
                local chosen_hand = pseudorandom_element(G.handlist, pseudoseed("star_d20roll6"))
                ...
                if (math.random(1, 2)) == 1 then
                    level_up_hand(nil, chosen_hand, nil, 2)
                else
                    level_up_hand(nil, chosen_hand, nil, -2)
                end
                update_hand_text({sound = 'button', volume = 0.7, pitch = 1.1, delay = 0}, {mult = 0, chips = 0, handname = '', level = ''})

            elseif card.ability.extra.roll == 7 then -- Multiply this blind's required score by 1.5 DONE
                SMODS.calculate_effect({message = "7", sound = 'star_coinheads', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)
                G.GAME.blind.chips = G.GAME.blind.chips * 1.5
                G.GAME.blind.chip_text = number_format(G.GAME.blind.chips)

            elseif card.ability.extra.roll == 8 then -- Reshuffle all jokers DONE
                SMODS.calculate_effect({message = "8", sound = 'star_coinheads', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)
                if #G.jokers.cards > 1 then
                    G.jokers:shuffle('aajk'); play_sound('cardSlide1', 0.85)
                    delay(0.15)
                    G.jokers:shuffle('aajk'); play_sound('cardSlide1', 1.15)
                    delay(0.15)
                    G.jokers:shuffle('aajk'); play_sound('cardSlide1', 1)
                    delay(0.5)
                end

            elseif card.ability.extra.roll == 9 then -- +1 luck, diminishes by 0.25 on round end  DONE
                SMODS.calculate_effect({message = "9", sound = 'star_coinheads', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)
                G.GAME.starcoluck = G.GAME.starcoluck + (1 - card.ability.extra.bonusluck)
                card.ability.extra.bonusluck = 1

            elseif card.ability.extra.roll == 10 then -- Does nothing DONE
                SMODS.calculate_effect({message = "10", sound = 'star_coinheads', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)
                SMODS.calculate_effect({message = "...", sound = 'star_boowomp', pitch = 1, volume = 0.7,}, card)

            elseif card.ability.extra.roll == 11 then -- Creates 2 random Skip Tags DONE
                SMODS.calculate_effect({message = "11", sound = 'star_coinheads', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)
                for i = 1, 2 do
                    G.E_MANAGER:add_event(Event({
                    trigger = 'after',
                    --delay = 0.4,
                    func = function()
                        ...
                        local tag_pool = get_current_pool('Tag')
                            local selected_tag = pseudorandom_element(tag_pool, pseudoseed('star_d20roll9'))
                            local it = 1
                            while selected_tag == 'UNAVAILABLE' do
                                it = it + 1
                                selected_tag = pseudorandom_element(tag_pool, pseudoseed('star_d20roll9'..it))
                            end
            ...
            elseif card.ability.extra.roll == 12 then -- Disables the Boss Blind, if there is one DONE
                SMODS.calculate_effect({message = "12", sound = 'star_coinheads', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)
            ...
            elseif card.ability.extra.roll == 13 then -- Multiply this blind's required score by 0.5 DONE
                SMODS.calculate_effect({message = "13", sound = 'star_coinheads', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)
                G.GAME.blind.chips = G.GAME.blind.chips * 0.5
                G.GAME.blind.chip_text = number_format(G.GAME.blind.chips)
              
            elseif card.ability.extra.roll == 14 then -- Copies the ability of the leftmost Joker DONE
                SMODS.calculate_effect({message = "14", sound = 'star_coinheads', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)

            elseif card.ability.extra.roll == 15 then -- Enhances all played cards with no enhancement into Lucky Cards DONE
                SMODS.calculate_effect({message = "15", sound = 'star_coinheads', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)
                local unenhanced = 0
                for _, scored_card in ipairs(context.scoring_hand) do
                    if scored_card.ability.set ~= 'Enhanced' and not scored_card.debuff then
            ...
            elseif card.ability.extra.roll == 16 then -- Gain +5 hand size next hand DONE
                SMODS.calculate_effect({message = "16", sound = 'star_coinheads', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)
                --card.ability.extra.handsizemod = card.ability.extra.handsizemod + 5
                G.hand:change_size(5)

            elseif card.ability.extra.roll == 17 then -- Win $20      DONE
                SMODS.calculate_effect({message = "17", sound = 'star_coinheads', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)  
                ease_dollars(card.ability.extra.moneymod)

            elseif card.ability.extra.roll == 18 then -- Retrigger all played cards twice DONE
                SMODS.calculate_effect({message = "18", sound = 'star_coinheads', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)
                card.ability.extra.retriggers = 2

            elseif card.ability.extra.roll == 19 then -- Regenerate either all hands or all discards DONE
                SMODS.calculate_effect({message = "19", sound = 'star_coinheads', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)
                G.E_MANAGER:add_event(Event {
                  trigger = "after",
                  delay = 0, 
                  func = function()
                        if (math.random(1, 2)) == 2 then 
                            ease_hands_played(G.GAME.round_resets.hands - G.GAME.current_round.hands_left, true, true)
                        else 
                            ease_discard(G.GAME.round_resets.discards - G.GAME.current_round.discards_left, true, true)
                        end
                    return true
                  end,
                })
                delay(0.5)
            else -- Permanently gain either +1 hand, +1 discard, +1 hand size, or +1 luck. Then, turn this Joker into a random held Joker
                SMODS.calculate_effect({message = "20", sound = 'star_coinheads', pitch = 1, volume = 0.7, colour = G.C.GREEN}, card)
                ...
                elseif choice == 2 then
                    ...
                    SMODS.calculate_effect({message = "+1 Discard", colour = G.C.RED}, card)
                elseif choice == 3 then
                    G.hand:change_size(1)
                    SMODS.calculate_effect({message = "+1 Hand Size"}, card)
                else
                    G.GAME.starcoluck = G.GAME.starcoluck + 1
                    SMODS.calculate_effect({message = "+1 Luck", colour = G.C.GREEN}, card)
                end
            end 
        end

        if context.repetition and card.ability.extra.retriggers then --if rolled 18
			if context.cardarea == G.play and card.ability.extra.retriggers > 0 then
				return {
					message = ("Again!"),
					repetitions = card.ability.extra.retriggers,
					card = card,
        ...
        -----AFTER ROUND-----
        if context.end_of_round and context.cardarea == G.jokers then --resetting things at end of round
            if card.ability.extra.handsizemod ~= 0 then
                G.hand:change_size(-1 * card.ability.extra.handsizemod)
                card.ability.extra.handsizemod = 0
            end

            if card.ability.extra.bonusluck > 0 then 
                card.ability.extra.bonusluck = card.ability.extra.bonusluck - 0.25 
                G.GAME.starcoluck = G.GAME.starcoluck - 0.25
                SMODS.calculate_effect({message = "-0.25 luck", colour = G.C.GREEN },  card)
            end 
        end
        ----POST HAND EFFECTS----
        if context.final_scoring_step then
        ...
        if context.after and context.cardarea == G.jokers then
            if card.ability.extra.rolled16 == true then --if last hand was a 16 get rid of the +5 hand size
                --card.ability.extra.handsizemod = card.ability.extra.handsizemod - 5
                G.hand:change_size(-5)
                card.ability.extra.rolled16 = false
            end

            if card.ability.extra.roll == 16 then 
                card.ability.extra.rolled16 = true
            end

            if card.ability.debuffedjoker then
                G.E_MANAGER:add_event(Event({
                trigger = 'after',
                delay = 0.4,
                func = function()
                    G.jokers.cards[card.ability.debuffedjoker]:juice_up()
                    G.jokers.cards[card.ability.debuffedjoker]:set_debuff(false)
                    card.ability.debuffedjoker = nil
                    return true
                    end
                }))
            end
            if card.ability.extra.roll == 20 then --destroying self and becoming new card
                G.E_MANAGER:add_event(Event({
                trigger = 'after',
                delay = 0.4,
                func = function()
                    if jokerCount() > 0 then
                        ...
                        G.E_MANAGER:add_event(Event {
                            ...
                            func = function() 
                                card:juice_up(juicesize,juicespin)
                                juicesound = juicesound + 0.05
                                juicespin = juicespin + 0.5
                                juicesize = juicesize + 0.1
                                play_sound('tarot2', juicesound)
                                return true
                            end,
                        })
                        end    
                        ...
                        local chosen_joker = pseudorandom_element(jokers, 'star_d20roll20')
                        local newcard = copy_card(chosen_joker, nil, nil, nil, chosen_joker.edition and chosen_joker.edition.negative)
                        card:set_ability(newcard.config.center.key, nil, true)
                        card:flip()
                        --SMODS.destroy_cards(card)
                        --SMODS.add_card { key = newcard.config.center.key, edition = card.edition }
